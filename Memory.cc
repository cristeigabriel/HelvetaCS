...
static SDK::ConVar* FindConVar(const Memory::Dll_t& module) {
	...
	constexpr uint8_t nIndice = (ServerBounded ? 0x68 : 0xE8);
	size_t iCount			  = 0u;

	//  Get next xref unless condition is met.
	while (*xref.Get<uint8_t*>(nPad) != nIndice) {
		++iCount;
...
Memory_t::Memory_t()
	: m_Client("client.dll")
	, m_Engine("engine.dll")
	, m_GameOverlayRenderer("gameoverlayrenderer.dll")
	, m_ShaderApiDx9("shaderapidx9.dll") {
	ADDRSET(
		this->m_HookFn,
		Hook_t,
		this->m_GameOverlayRenderer.FindString<"Couldn't get trampoline region lock, will continue possibly unsafely.\n", true>(0xA1, 12).FollowUntil(
			0x55,
			false));
	...
	ADDRSET_PAD(
		this->m_pClientClassHead,
		SDK::ClientClass***,
		1,
		**this->m_Client.FindString<"Unknown entity class received in ProcessSpottedEntityUpdate.\n", true>().FollowUntil(0xA1, false));
	ADDRSET_PAD(
		this->m_pGlobalVars,
		SDK::CGlobalVarsBase**,
		1,
		*this->m_Engine.FindString<"VClientDllSharedAppSystems001", false>().FollowUntil(0x68, false).FollowUntil(0x68, false));
	...
	ADDRSET_PAD(
		this->m_pEntityList,
		SDK::IClientEntityList**,
		1,
		*this->m_Client.FindPattern(STB("B9 ? ? ? ? FF 10 85 C0 74 0B 8B 10 8B C8 FF 74 F7 04 FF 52 14")));
	...
	ADDRSET_PAD(this->m_pEngineClient, SDK::IVEngineClient***, 1, **this->m_Client.FindString<"VEngineClient014", true>().FollowUntil(0xA3, true));
